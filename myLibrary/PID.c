...
void PID_Init(PID_typedef *pid, float kp, float ki, float kd, float limit) {
    ...
    pid->integral_limit = limit * 0.5f; // Ch?n tích phân ? 50% công su?t
		pid->last_pos = 0;
    PID_Reset(pid);
		pid->intergral_vel_limit = limit *1.0f;
}
...
float PID_ComputeVel(PID_typedef *pid, float SetPoint, float MeasuredValue, float dt) {
    pid->error = SetPoint - MeasuredValue;
    ...
    // Thành ph?n I (Quan tr?ng nh?t d? gi? v?n t?c ?n d?nh khi có t?i)
    pid->integral_Stored += pid->error * dt;
    if (pid->integral_Stored > pid->intergral_vel_limit) pid->integral_Stored = pid->intergral_vel_limit;
    else if (pid->integral_Stored < -pid->intergral_vel_limit) pid->integral_Stored = -pid->intergral_vel_limit;
    float I = pid->Ki * pid->integral_Stored;
